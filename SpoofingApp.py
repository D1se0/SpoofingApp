...
# Verificacion de ejecuccion como root
def check_root():
    if os.geteuid() != 0:
...
# --- Título animado ---
def animate_title(text, delay=100):
    def anim(i=0):
        if i <= len(text):
            app.title(text[:i])
            app.after(delay, anim, i+1)
    anim()
...
# Animar título al iniciar
animate_title("Pentest ARP/DNS Spoof Tool")
...
style.configure("Treeview",
                background=BG_COLOR,
                foreground=NEON_MAGENT,
                fieldbackground=BG_COLOR,
                font=PRESS_START_FONT)
style.configure("Treeview.Heading",
                background=BG_COLOR,
                foreground=NEON_PURPLE_WHITE,
...
# --- Marco Configuración ---
frame_config = tk.LabelFrame(app, text="Config", fg=NEON_ORANGE, bg="black", font=PRESS_START_FONT_BOLD, labelanchor="n", padx=15, pady=10)
frame_config.pack(pady=15, padx=15, fill="x")

tk.Label(frame_config, text="IP de la víctima:", fg=NEON_PURPLE_WHITE, bg="black", font=PRESS_START_FONT).grid(row=0, column=0, sticky="w", pady=6)
victim_ip_entry = tk.Entry(frame_config, width=20, bg="black", fg=NEON_MAGENT, insertbackground='lime', font=PRESS_START_FONT)
victim_ip_entry.grid(row=0, column=1, padx=(5, 30), pady=6)

tk.Label(frame_config, text="Dominios a Spoofear:", fg=NEON_PURPLE_WHITE, bg="black", font=PRESS_START_FONT).grid(row=1, column=0, sticky="w", pady=6)
domains_entry = tk.Entry(frame_config, width=40, bg="black", fg=NEON_MAGENT, insertbackground='lime', font=PRESS_START_FONT)
...
log_box = scrolledtext.ScrolledText(log_frame, bg="black", fg=NEON_MAGENT, font=PRESS_START_FONT, height=10, insertbackground="lime", spacing3=6)
